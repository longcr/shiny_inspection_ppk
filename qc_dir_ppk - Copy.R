...
qqplot_fn <- function(fndatnorm){
  ...
  xlab = max(fndatnorm$value)
  density(fndatnorm$value)
  str(ggout)
  
  ggout <- ggout + 
    geom_text(data = aes(label = text), vjust = "inward", hjust = "inward")
...
# verify
# plot(d_dat_nest2$plot[[3]])

qqplot_fn(d_dat_nestj$data[[1]])
